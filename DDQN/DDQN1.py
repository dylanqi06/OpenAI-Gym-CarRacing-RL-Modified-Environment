...
PRETRAINED_PATH         = "model/dylanqi/DDQN1_81b96/20241122-122922/episode_370.weights.h5"
...
class DDQN_Agent:
    ...
    def save( self, name, rewards ):
        ...
        self.target_model.save_weights(MODEL_DIR + name + ".weights.h5")
...
def test_agent( agent : DDQN_Agent, env : gym.make, model : str ):
    ...
    for i in range(10):
        ...
        print("[INFO]: For the ",i, "-th run, Run Reward: ", sum_reward, " | Time:", "%0.2fs."%t1 )
    print(f"Average Reward: {np.mean(run_rewards)}, Max: {np.max(run_rewards)}, Min: {np.min(run_rewards)}")
