...
EPISODES                = 2001      # training episodes
...
class DDQN_Agent:
    def __init__(   self, 
        ...
        self.action_space = [ 
            (-action[0], action[2], action[1]) for action in action_space
        ]
    ...
    for episode in tqdm( range(episodes) ):
        ...
        while not done and sum_reward > MAX_PENALTY and can_see_road:# and not car_on_grass:
            ...
            for _ in range( SKIP_FRAMES + 1 ):
                ...
                # reward += min(0.1 * speed, 1)  # Cap the speed reward at 1

                # if speed < 0.1:  # Low speed threshold
