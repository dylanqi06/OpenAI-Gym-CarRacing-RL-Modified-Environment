...
mixed_precision.set_global_policy('mixed_float16')  # Comment this line if not using mixed precision
...
class DDQN_Agent:
    ...
    @tf.function
    def choose_action_tf(self, state):
        """TensorFlow graph-compatible action selection."""
        q_values = self.model(state, training=False)
        action_idx = tf.argmax(q_values[0])
        return action_idx
    ...
    def choose_action(self, state, best=False, _random=False):
        ...
        actionIDX = self.choose_action_tf(state_tensor).numpy()
